'use strict';
innoApp
  .controller('fttabCtrl', ["$scope", "AuthService", "ajaxFactory", function($scope, AuthService,ajaxFactory) {
    $scope.role = AuthService.getRole();
    $scope.tab = 'fertilitytechnology';
    $scope.$parent.active = 'FT';
    $scope.loading=true;
    ...
    $scope.$watch('filterObj', function() {
      ...
      filterdata().then(function(filterResults) {
        ...
        var rhdata = function() {
          return ajaxFactory.getData("/v1/sales/ta", "post", "json", {"ta":"FT", "filters" : $scope.filterObj},null);
        }
        rhdata().then(function(res) {
          ...
          var salesperformancerhdata = function() {
            return ajaxFactory.getData("/v1/sales/company/salesperformance", "post","json", {"ta":"FT", "filters" : $scope.filterObj},null);
          }
